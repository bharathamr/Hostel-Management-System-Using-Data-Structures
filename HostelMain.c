	...
	while(1)
	{
		...
		if(choice == 8)
		{
			...
			if(!isEmpty(single))
			{
				...
				temp = single->next;
				int count = 0;
				while(temp!=NULL && count != singlecount)
				{
					count++;
			...
			if(!isEmpty(shared))
			{
				...
				temp = shared->next;
				int count = 0;
				while(temp!=NULL && count != sharecount)
				{
					count++;
					fwrite(temp, sizeof(struct node), 1, fp);
					temp = temp->next;
			...
			if(!isEmpty(inter))
			{
				...
				temp = inter->next;
				int count = 0;
				while(temp!=NULL && count != intercount)
				{
					count++;
					fwrite(temp, sizeof(struct node), 1, fp);
					temp = temp->next;
